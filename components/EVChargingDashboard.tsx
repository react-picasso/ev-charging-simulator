	...
	useEffect(() => {
		...
		const generateMockDayData = (): PowerData[] => {
			...
			for (let hour = 0; hour < 24; hour++) {
				...
				const activeChargers = Math.round(
					numChargePoints * arrivalProb * (1 + Math.random() * 0.5)
				);
				let totalPower = 0;
			...
			setMockStats((prev) => ({
				...
				concurrencyFactor: Math.round(
					(maxPowerUsed / theoreticalMax) * 100
				),
		...
		const generateMockStats = (): SimulationStats => {
			const dailyArrivals = Object.values(HOURLY_ARRIVAL_PROBS).reduce(
				(sum, prob) =>
					sum +
					prob * numChargePoints * 24 * (arrivalMultiplier / 100),
				0
			);
		...
		const generateChargepointData = (): ChargepointData[] => {
			// Calculate base average charging time (in hours) per charging session
			const calculateAvgChargingTime = () => {
				const chargingProbability = 1 - CHARGING_DEMANDS[0].probability;

				const totalChargingTime = CHARGING_DEMANDS.reduce(
					(sum, demand) => {
						if (demand.range === 0) return sum;
						const energyNeeded = (demand.range * consumption) / 100;
						const chargingTime = energyNeeded / chargingPower;
						return (
							sum +
							chargingTime *
								(demand.probability / chargingProbability)
						);
					},
					0
				);
			...
			const baseChargingTime = calculateAvgChargingTime();
			const chargingProbability = 1 - CHARGING_DEMANDS[0].probability;

			// Calculate base utilization from arrival patterns
			const calculateBaseUtilization = () => {
				const dailyUtilization = Object.values(
					HOURLY_ARRIVAL_PROBS
				).reduce((sum, prob) => {
					const adjustedProb = prob * (arrivalMultiplier / 100);
					const hourlyUtilization =
						adjustedProb * chargingProbability * baseChargingTime;
					return sum + hourlyUtilization;
				}, 0);

				// Convert to percentage but don't cap yet - we'll add variation first
				return dailyUtilization * 100;
			};

			const baseUtilization = calculateBaseUtilization();

			// Generate data for each chargepoint with individual variations
			return Array.from({ length: numChargePoints }, (_, i) => {
				// Add random variation to utilization (-20% to +20% of base value)
				const variationFactor = 0.8 + Math.random() * 0.4; // Random factor between 0.8 and 1.2
				const utilization = Math.min(
					100,
					Math.round(baseUtilization * variationFactor)
				);

				// Calculate average power with similar variation
				const avgPowerBase =
					CHARGING_DEMANDS.reduce((sum, demand) => {
						if (demand.range === 0) return sum;
						const powerNeeded = Math.min(
							chargingPower,
							(demand.range * consumption) / 100
						);
						return sum + powerNeeded * demand.probability;
					}, 0) /
					(1 - CHARGING_DEMANDS[0].probability);

				// Apply similar variation to average power
				const averagePower =
					Math.round(
						((avgPowerBase * utilization) / 100) *
							variationFactor *
							10
					) / 10;
		...
		const generateEventsData = (): EventData[] => {
			const baseEventsPerDay = Object.values(HOURLY_ARRIVAL_PROBS).reduce(
				(sum, prob) =>
					sum +
					prob * numChargePoints * 24 * (arrivalMultiplier / 100),
				0
				...
				case "day":
					for (let hour = 0; hour < 24; hour++) {
						data.push({
							...
							events: Math.round(
								baseEventsPerDay * HOURLY_ARRIVAL_PROBS[hour]
							),
						});
					}
					break;
